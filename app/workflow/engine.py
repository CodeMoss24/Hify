...
class WorkflowEngine:
    ...
    async def _execute_llm_node_stream(
        self, db: Session, node: WorkflowNodeModel, ctx: ExecutionContext
    ) -> AsyncGenerator[str, None]:
        """流式执行 LLM 节点，逐 token yield delta

        与 _execute_llm_node 逻辑一致，但不收集全量响应，而是实时 yield。
        """
        config = node.config or {}
        model_config_id = config.get("model_config_id")
        prompt_template = config.get("prompt", "")
    ...
    async def execute_stream(
        ...
        """流式执行工作流，逐 token yield event dict

        每个 LLM 节点先 yield 进度标记，再逐 token yield delta。
        所有 event 类型为 'delta'，前端无需改造即可渲染。
            ...
            while current_node_key and step_count < max_steps:
                ...
                try:
                    ...
                    if current_node.node_type == "LLM":
                        ...
                        node_output = ""
                        async for delta in self._execute_llm_node_stream(db, current_node, ctx):
                            node_output += delta
                    ...
                    # 更新节点执行记录
                    elapsed_ms = int((time.monotonic() - node_start) * 1000)
                    output_data = {"result": node_output} if (current_node.node_type == "LLM" and node_output) else None
                    self._update_node_run(db, node_run.id, "SUCCESS", None, output_data, elapsed_ms)
